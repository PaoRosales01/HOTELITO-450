...
if  (isset($_GET['id_reserva'])) {
  ...
  if (mysqli_num_rows($resultado) == 1) {
    ...
    $hrentrada=$fila['Hr_Entrada'];
    $fecha_ent=$fila['Fecha_Entrada'];
    $fecha_sal=$fila['Fecha_Salida'];
    $estado=$fila['Estado'];
    $hab=$fila['No_habitacion'];
...
if (isset($_POST['update'])) {
  ...
  $sql = "UPDATE reservacion set Hr_Entrada = '$hrentrada', Fecha_Entrada = '$fecha_ent', Fecha_Salida = '$fecha_sal', Estado = '$estado', 
          No_habitacion = '$hab', Usuario = '$usuario' WHERE Id_Reservacion = $id";
...
<div class="se-pre-con"></div>
    <div class="wrapper">
        ...
        <div id="content">
            ...
            <div align="center">
  <h1 class="tab-titulo ">Editar Reserva</h1>
</div>
...
<div class="container p-4">
  <div class="row">
    <div class="col-md-4 mx-auto">
      ...
      <form action="editar_reserva.php?id_reserva=<?php echo $_GET['id_reserva']; ?>" method="POST">
      <label class="desc">Hora de entrada:</label>
      <div class="form-group">
      <input id="timepicker" width="276" class="form-control" name="hrent" value="<?php echo $hrentrada;?>" autofocus required readonly>
            <script>
        ...
        <div class="form-group">
        <input type="date" name="fecha_entrada" class="form-control" placeholder="Fecha de entrada" value="<?php echo $fecha_ent; ?>" required>
        </div>
        <label class="desc">Fecha de salida:</label>
        <div class="form-group">
        <input type="date" name="fecha_salida" class="form-control" value="<?php echo $fecha_sal; ?>" required>
        </div>
        ...
        <div class="form-group">
        <input type="number" name="habitacion" class="form-control" min="1" max="3" value="<?php echo $hab;?>" required>
        </div>
        <label class="desc">No. de usuario</label>
        <div class="form-group">
        <input type="number" name="usuario" class="form-control" placeholder="Número de usuario" value="<?php echo $usuario;?>" required>
        </div>
        <input type="reset" name="borrar" class="btn btn-danger" value="Borrar">
        <input type="submit" name="update" class="btn btn-success" value="Actualizar Reserva">      </form>
      </div>
    </div>
  </div>
</div>
...
if ($msj=="actualizado"){
      ...
      confirmButtonText: 'Continuar',
			...
			window.location = "admin_reservas.php";
		});
		}
		mensaje();
		</script>
