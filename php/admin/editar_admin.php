...
if (isset($_POST['update'])) {
  ...
  $sql = "UPDATE administrador set  Correo = '$correo', nombre = '$nombre', apellido = '$apellido', contrasenia= '$contraseña' WHERE Id_admin = $id";
  $resultado=mysqli_query($conexion, $sql);
  if(!$resultado) {
    $msj="error";
  }
...
<div class="se-pre-con"></div>
    <div class="wrapper">
        ...
        <div id="content">
            ...
            <div align="center">
  <h1 class="tab-titulo ">Editar Administrador</h1>
</div>
...
<div class="container p-4">
  <div class="row">
    <div class="col-md-4 mx-auto">
      ...
      <form action="editar_admin.php?id_admin=<?php echo $_GET['id_admin']; ?>" method="POST">
      
      <label class="desc">Correo Electrónico:</label>
        <div class="form-group">
            <input type="email" name="correo" value="<?php echo $correo; ?>" class="form-control" autofocus required>
          </div>
          <label class="desc">Nombre:</label>
          <div class="form-group">
            <input type="text" name="nombre" value="<?php echo $nombre; ?>" class="form-control" autofocus required>
          </div>
          <label class="desc">Apellido:</label>
          <div class="form-group">
            <input type="text" name="apellido" value="<?php echo $apellido; ?>" class="form-control" autofocus required>
          </div>
          ...
          <input type="submit" name="update" class="btn btn-success" value="Actualizar Admin">
        </form>
      </div>
    </div>
      </div>
		...
		function mensaje(){
 		 swal({
			imageUrl: ('../../img/cambio.png'),
      		title: '- Admin Actualizado Exitosamente -',
			imageWidth: 250,
			imageHeight: 200,
			padding: 10,
			animation: true,
